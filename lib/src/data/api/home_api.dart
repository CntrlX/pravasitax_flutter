...
class HomeAPI {
  ...
  DateTime? _parseDateTime(dynamic dateData) {
    try {
      ...
      if (dateData is Map && dateData['\$date'] is Map) {
        ...
        if (longValue != null) {
          return DateTime.fromMillisecondsSinceEpoch(int.parse(longValue.toString()));
        }
      }
      return null;
    } catch (e) {
  ...
  Future<HomePageData> getHomePageData() async {
    try {
      ...
      developer.log('API Response Status Code: ${response.statusCode}', name: 'HomeAPI');
        ...
        if (responseData['response'] == '200') {
          try {
            ...
            final Map<String, dynamic> transformedData = {
              'serviceList': (responseData['data']['serviceList'] as List?)?.map((item) => {
                'service': item['service'] ?? '',
                'url': item['url'],
              }).toList() ?? [],
              'scenarios': (responseData['data']['scenarios'] as List?)?.map((item) => {
                'scenario': item['scenario'] ?? '',
                'url': item['url'],
              }).toList() ?? [],
              'firstSectionBanners': (responseData['data']['firstSectionBanners'] as List?)?.map((item) => {
                'title': item['title'] ?? '',
                'label': item['label'] ?? '',
                'url': item['url'],
                'image': item['image'] ?? '',
              ...
              'taxTools': (responseData['data']['taxTools'] as List?)?.map((item) => {
                'title': item['title'] ?? '',
                'url': item['url'],
              }).toList() ?? [],
              'event': responseData['data']['event'] != null ? {
                ...
                'location': responseData['data']['event']['location'] ?? '',
                'price': responseData['data']['event']['price'],
              } : null,
              'blogs': (responseData['data']['blogs'] as List?)?.map((item) => {
                'id': item['_id'] ?? '',
                'title': item['title'] ?? '',
                'url': item['url'],
            ...
            developer.log('Transformed data: $transformedData', name: 'HomeAPI');
            
            final homeData = HomePageData.fromJson(transformedData);
            developer.log('Successfully parsed home page data', name: 'HomeAPI');
            return homeData;
          } catch (e, stackTrace) {
            developer.log(
